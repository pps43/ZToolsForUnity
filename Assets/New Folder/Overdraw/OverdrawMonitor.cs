...
public class OverdrawMonitor : MonoBehaviour
{
		...
		get
		{
			if (instance == null)
			{
				...
				if (instance == null)
				{
                    ...
                    Debug.Log("create OverdrawMonitor");
    ...
    private string _dstCameraName = "BG Camera";
    private Camera _dstCamera;//camera to be analyzed
	...
	public void Awake()
	{
        ...
        if(_dstCamera == null)
        {
            _dstCamera = GameObject.Find(_dstCameraName).GetComponent<Camera>();
        }
	...
	{
        Debug.Log("[ overdraw monotor enable ]");
		disabled = false;
	}

	public void OnDisable()
	{
        Debug.Log("[ overdraw monotor diable ]");
        disabled = true;
		OnDestroy();
	}
	...
	public void LateUpdate()
	{
        ...
        //Camera dstCamera = Camera.main;
        if (_dstCamera == null && !string.IsNullOrEmpty(_dstCameraName))
        {
            _dstCamera = GameObject.Find(_dstCameraName).GetComponent<Camera>();
        }
        if(_dstCamera == null) { return; }
		...
		transform.position = _dstCamera.transform.position;
		transform.rotation = _dstCamera.transform.rotation;

		RecreateTexture(_dstCamera);
	...
	private void RecreateTexture(Camera dstCamera)
	{
		...
		if (dstCamera.pixelWidth != overdrawTexture.width || dstCamera.pixelHeight != overdrawTexture.height)
		{
			overdrawTexture.Release();
			overdrawTexture.width = dstCamera.pixelWidth;
			overdrawTexture.height = dstCamera.pixelHeight;
		}
	}
    ...
    public void setCameraToAnalysis(string cameraName)
    {
        _dstCameraName = cameraName;
    }
